...
- 成員：`/hearthkeeper` → 提出回報 → 選匿名或私密 → 填表。
- 查詢：`/myreports` 查看自己的案件、補充內容及申請結案／重開。
- 社管：私人案件論壇的「處理案件」或 `/cases` → 認領、回覆、結案留檔。
- `/ping` 檢查連線；案件功能須完成私人論壇與角色設定才啟用。

回覆只對操作本人可見。匿名模式隱藏向社管提供的帳號，機器人仍保存加密身分對應。
案件結案後保存 90 天；一般論壇聊天不會轉送給成員。
機器人不讀取一般聊天、不需要 AI。檢舉／申訴的獨立承辦組、通知身分組及處分功能尚未提供。
現行範圍與限制見 [0.2 回報流程](docs/technical-design/Feedback_0.2.md)。
