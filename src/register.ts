...
import { commandsFor } from "./runtime.js";
...
async function register() {
  ...
  const expected = commandsFor(config)
    .map((x) => x.name)
    .sort();
  if (
    JSON.stringify(saved.map((x) => x.name).sort()) !== JSON.stringify(expected)
  )
    throw new Error("Readback mismatch");
  console.log(
    JSON.stringify({ event: "commands_verified", count: saved.length }),
  );
}
register().catch(() => {
